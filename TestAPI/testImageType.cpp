...
BOOL testLoadSaveConvertImageType(FIBITMAP *src, FREE_IMAGE_TYPE image_type) {
	...
	try {
		...
		// save image as PNG
		bResult = FreeImage_Save(FIF_PNG, dst, "TestImageType.png", PNG_DEFAULT);
		if(!bResult) throw(1);
		...
		// load image
		dst = FreeImage_Load(FIF_PNG, "TestImageType.png", PNG_DEFAULT);
		if(!dst) throw(1);
		...
		// save image as PNG
		bResult = FreeImage_Save(FIF_PNG, chk, "TestImageType.PNG", PNG_DEFAULT);
		if(!bResult) throw(1);
